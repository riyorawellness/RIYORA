 ...
 *   3. Handler / dismiss → /payments/subscription/{sid}/verify polls
 *      Razorpay for the authoritative status. If mandate authenticated
 *      AND first charge captured, the backend materialises a
 *      program_purchases row and returns { purchase_id, expiry_date }.
 *
 * Props: open, onOpenChange(bool), programId, onSuccess({...})
 */
export default function SubscriptionCheckoutModal({
  ...
  const [session, setSession] = useState(null); // init response
  ...
  const pollVerify = async () => {
    setStatus("processing");
    // Poll up to 8 × 2s = 16s to give Razorpay's `subscription.charged`
    // webhook time to arrive (it typically fires within 3-5s of mandate
    // authentication in India).
    for (let attempt = 0; attempt < 8; attempt++) {
      try {
        const res = await paymentsApi.subscriptionVerify(session.subscription_id);
        // Treat both purchase_id present OR status active/authenticated as success.
    ...
    // Timed out — the mandate may still authenticate. Let the user retry.
    setStatus("ready");
    setError(
      "If you approved the mandate, your access will unlock automatically within a minute. Otherwise, tap Subscribe again.",
    );
  };
  ...
  const runLive = async () => {
    ...
    const opts = {
      ...
      retry: { enabled: false },
      handler: async () => {
        // Razorpay reports mandate accepted — reconcile with our backend.
        pollVerify();
      },
      modal: {
        // If the user closes the modal mid-flow, still reconcile — the
        // mandate could have been authenticated & the first cycle charged
        // even if Checkout showed a failure state.
        ondismiss: () => {
          if (status === "processing") return;
          pollVerify();
        },
          ...
          <>
            ...
            <div className="mt-5 space-y-2 rounded-2xl bg-neutral-50 p-4 text-sm">
              ...
              {breakdown.discount > 0 && (
                <Row k="Discount" v={`- ${inr(breakdown.discount)}`} />
              )}
              <Row k="Sub-total" v={inr(breakdown.taxable)} />
              <Row
                k={`GST @ ${breakdown.gst_percent ?? 18}%`}
                v={inr(breakdown.gst_amount)}
              />
            ...
            <div
              className="mt-4 flex items-start gap-2 rounded-xl border border-[hsl(var(--rw-royal))]/20 bg-[hsl(var(--rw-sky-soft))] p-3 text-xs text-[hsl(var(--rw-royal-deep))]"
            >
              <Repeat className="mt-0.5 h-3.5 w-3.5" />
              <div>
                First cycle is charged as soon as you approve the mandate. Every
                subsequent cycle debits automatically until you cancel.
              </div>
            </div>
            ...
            {session.is_mock && (
              ...
              >
                ...
                <div>
                  <b>Sandbox mode.</b> No real mandate — Razorpay keys are not
                  configured. Tap Subscribe to simulate a successful mandate.
                </div>
              </div>
            )}
