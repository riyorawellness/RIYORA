...
/**
 * MySubscriptions — user-facing list of Razorpay AutoPay mandates.
 *
 * Backed by:
 *   GET  /api/payments/subscription/me                    (list)
 *   POST /api/payments/subscription/{sid}/cancel          (cancel)
 */
const STATUS_STYLES = {
...
function frequencyLabel(freq) {
  const map = { monthly: "Monthly", half_yearly: "Half-yearly", yearly: "Yearly" };
...
function formatDate(v) {
  if (!v) return "—";
  try {
    // Backend stores ISO strings for created_at/updated_at etc.; Razorpay
    // fields (current_start / current_end / last_charged_at) may be Unix ints.
    const d =
      typeof v === "number"
        ? new Date(v * 1000)
        : new Date(v);
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            <span className="text-[11px] text-muted-foreground">
              {frequencyLabel(sub.frequency)} · {formatINR(amount)} / cycle
            </span>
          </div>
          {sub.is_mock && (
...
function ConfirmDialog({ open, sub, onConfirm, onClose, working }) {
  ...
  const isPending = String(sub.status || "").toLowerCase() === "created" || String(sub.status || "").toLowerCase() === "pending";
  return (
    <div
  ...
  const doCancel = async () => {
    ...
    try {
      ...
      if (res.already_terminal) {
        toast.info(`Subscription is already ${res.status}.`);
      } else if (res.cancel_at_cycle_end) {
        toast.success("Cancellation scheduled — access continues until the current cycle ends.");
      } else {
  ...
  return (
    <div className="px-5 pb-24 pt-4" data-testid="my-subscriptions-page">
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Manage the Razorpay UPI mandates that renew your subscription programs each cycle.
      ...
      {loading ? (
        ...
        <div className="mt-6 rounded-2xl border border-dashed p-8 text-center" data-testid="subscriptions-empty">
          ...
          <button
            ...
            className="mt-4 rw-btn-pill rw-btn-primary"
            data-testid="subscriptions-browse"
          >
            Browse programs
          </button>
        </div>
